    ...
    static send(chatId, from, type, content) {
        return new Promise((resolve, reject) => {
            ...
            addDoc(Message.getRef(chatId), {
                ...
                const docRef = doc(Message.getRef(chatId), result.id);
                setDoc(docRef, { status: 'sent' }, { merge: true }).then(() => resolve(docRef)).catch(err => reject(err));
            }).catch(err => {
                reject(err);
            });
        });
